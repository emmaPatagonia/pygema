...
def plot_seismicity_time_serie(starttime, endtime, dark_background=True, show_plot=False, save_plot=True, savedir=None, format='jpg', dpi=150):
  ...
  ax1.set_title("last update: %s UTC" % (UTCDateTime().strftime("%Y/%m/%d %H:%M:%S")), fontsize=6, loc='right')
  for ax in [ax1,ax2]:
    ax.minorticks_on()
    ax.tick_params(axis='both', which='major', labelsize=8, bottom='off', top='off', left='on', right='on', direction='in')
    ax.tick_params(axis='both', which='minor', labelsize=8, bottom='off', top='off', left='on', right='on', direction='in')
  ...
  ax1.set_ylabel(r"N$^\circ$ events per day")
