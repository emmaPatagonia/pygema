...
def plot_map(elon, wlon, nlat, slat, res='c', dpi=300, xpixels=800, add_holocene_volcanoes=False, add_seismic_stations=False, add_ralco=False, add_labels=False, add_faults=False, add_events_pygemadb=False, dark_background=False, show_plot=True, savedir=None):
  ...
  if add_holocene_volcanoes:
    ...
    if add_labels:
      for x,y,vname in zip(xvolcs,yvolcs,vnames):
        if vname == 'Tolhuaca':
          ax.annotate('V. '+vname+'\n', (x, y), color='k', weight='bold', fontsize=5.5, ha='center', va='bottom', clip_on=True, zorder=2000+3, fontstyle='normal')
        elif vname == 'Copahue':
          ax.annotate('   V. '+vname, (x, y), color='k', weight='bold', fontsize=5.5, ha='left', va='top', clip_on=True, zorder=2000+3, fontstyle='normal')
        elif vname == 'Callaqui':
          ax.annotate(' V. '+vname, (x, y), color='k', weight='bold', fontsize=5.5, ha='left', va='bottom', clip_on=True, zorder=2000+3, fontstyle='normal')
        elif vname == 'Trolon':
          ax.annotate('V. '+vname+' ', (x, y), color='k', weight='bold', fontsize=5.5, ha='right', va='bottom', clip_on=True, zorder=2000+3, fontstyle='normal')
  ...
  if add_seismic_stations:
    ...
    for net, stat, lon, lat in zip(networks, stations, stlons, stlats):
      ...
      if add_labels:
        ax.annotate(stat, (x, y), weight='bold', fontsize=5, ha='left', va='center', clip_on=True, zorder=2000+3)
  ...
  if add_ralco:
    ...
    if add_labels:
      ax.annotate('Pangue  ',xy=(si,sj), ha='right', va='top', color='k',zorder=15,fontsize=6,fontweight='bold',xytext=(-0,-0),textcoords='offset points', fontstyle='normal', clip_on=True)

    si,sj = map(-71.475571, -38.046040)
    map.scatter(si, sj, c=color_places, linewidths=0.3, edgecolors='k', alpha=alpha, zorder=15, marker='X', s=40, clip_on=True)
    if add_labels:
      ax.annotate('Ralco  ',xy=(si,sj), ha='right', va='top', color='k',zorder=15,fontsize=6,fontweight='bold',xytext=(-0,-0),textcoords='offset points', fontstyle='normal', clip_on=True)
  ...
  if add_events_pygemadb:
    ...
    ax.scatter(x,y,c=z, cmap=cmap, s=s, marker='o', norm=norm, zorder=100, alpha=1, clip_on=True, lw=0.3, edgecolors='k')
